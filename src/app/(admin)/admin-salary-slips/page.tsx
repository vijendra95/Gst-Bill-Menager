...
export default function AdminSalarySlipsPage() {
  ...
  const loadData = async (uid: string) => {
    ...
    setLoading(true);
    const res = await fetch(`/api/salary-slips?adminUserId=${uid}`);
  ...
  const totalPaid = filtered.filter((i) => i.status === "paid").reduce((s, i) => s + i.netSalary, 0);

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><CreditCard className="w-6 h-6 text-indigo-600" /> Salary Slips (Admin)</h1>
      ...
      {selectedClient && (
        <>
          <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div className="rounded-xl p-4 bg-white border shadow-sm">
              <p className="text-xs text-gray-500">Slips</p>
              <p className="text-lg font-bold text-gray-900">{filtered.length}</p>
            </div>
            <div className="rounded-xl p-4 bg-white border shadow-sm">
              <p className="text-xs text-gray-500">Pending</p>
              <p className="text-lg font-bold text-orange-600">{filtered.filter((i) => i.status === "draft").length}</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400">No salary slips found</div>
          ) : (
            <div className="bg-white border rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50 border-b text-left text-gray-500">
                  <th className="px-4 py-3">Employee</th><th className="px-4 py-3">Month</th><th className="px-4 py-3">Net Pay</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Date</th>
                </tr></thead>
                <tbody>
                  {filtered.map((i) => (
                    <tr key={i.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-semibold">{formatCurrency(i.netSalary)}</td>
                      <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${i.status === "paid" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{i.status}</span></td>
      ...
      {!selectedClient && !loading && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p>Select a client to view their salary slips</p>
        </div>
      )}
    </div>
  );
